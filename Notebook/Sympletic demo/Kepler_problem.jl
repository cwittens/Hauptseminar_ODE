...
# ╔═╡ 6d471f5e-b8ed-4b72-a451-3721277c3e16
...
md"""
# Kepler Problem und Symplektische Verfahren
Der Code bezüglich des Kepler Problems ist inspiriert von [Yingbo Ma and Chris Rackauckas](https://docs.sciml.ai/SciMLTutorialsOutput/html/models/05-kepler_problem.html).
"""
...
Der Hamiltonien $$\mathcal{H}$$ und der Drehimpuls $$L$$ sind Erhaltungsgrößen für das Kepler Problem und sind gegeben als:
...
html"""
<center>
<p><a href="https://commons.wikimedia.org/wiki/File:Classical_Kepler_orbit_e0.6_light.gif#/media/Datei:Classical_Kepler_orbit_e0.6_light.gif"><img src="https://upload.wikimedia.org/wikipedia/commons/5/56/Classical_Kepler_orbit_e0.6_light.gif" alt="Classical Kepler orbit e0.6 light.gif" height="300" width="400"></a><br> Von <a href="//commons.wikimedia.org/wiki/User:WillowW" title="User:WillowW"&gt;WillowW&lt;/a&gt; - &lt;span class="int-own-work" lang="de"&gt;Eigenes Werk&lt;/span&gt;, <a href="https://creativecommons.org/licenses/by/3.0" title="Creative Commons Attribution 3.0">CC BY 3.0</a>, <a href="https://commons.wikimedia.org/w/index.php?curid=3372521">Link</a></p></center>
"""
...
md"""
### Vielleicht haben wir ja mehr Glück mit einem impliziten Rosenbrock Verfahren (`ROS3`)
"""

# ╔═╡ 3c55ea76-15b4-4854-8ed7-ae163f3e793f
md"""
### Das sieht erstmal besser aus, auf lange Sicht haben wir jedoch trotzdem einen Drift der uns zum Verhängnis wird.
"""

# ╔═╡ d928e6a6-1181-44b7-bf39-57a77ab63397
md"""
### Da kommen nun Symplektische Verfahren ins Spiel, in diesem Fall (`Ruth3`).
"""
...
md"""
## Betrachten wir nun den Fehler verschiedener Verfahren auf die analytische Lösung.
"""

# ╔═╡ 791d9eca-c0b6-4430-bc17-74289d2f8182
md"""
### Betrachten wir wieder zuerst das explizite Verfahren (`Tsit5`). Man erkennt schnell, dass der Fehler quadratisch mit der Zeit wächst.
"""

# ╔═╡ b127bec3-a4db-4666-acfb-c5b70f67efe4
md"""
### Dasselbe quadratische Verhalten des Fehlers sehen wir auch beim impliziten Rosenbrock Verfahren (`ROS3`).
"""

# ╔═╡ dd63eb69-e061-4d18-b0f7-d2383b53cefb
md"""
### Im Gegensatz dazu besitzen Symplektische Verfahren ein fundamental anderes Fehlerverhalten.
"""

# ╔═╡ 90e558c1-fcdf-4e4a-98aa-dfa5a9de6d51
md"""
### Auf langen Zeitskalen sehen wir, dass es sich um ein lineares Fehlerwachstum handelt.
"""

# ╔═╡ b1184e4d-0f9d-4e35-81f2-d64804959d45
md"""
# Kurze Wiederholdung: Der Vorteil von Schrittweitenkontrolle
"""

# ╔═╡ ed508b1c-c1f9-4020-b7a2-d61009462090
md"""
## Wir betrachten Trajektorie eines Satellitens im Gravitationsfeld der Erde und des Mondes.
Der Code bezüglich der Schrittweitenkontroll-Demo ist übernommen von [Hendrik Ranocha](https://youtu.be/yMR45cZbvDw?si=RksNJ-T81bhxlZai).
"""

# ╔═╡ 84724946-469e-44ff-acbe-18643751b0ae
md"""
...
# ╔═╡ d0060e2e-3b50-41e7-979f-a41468c31ebe
md"""
`tol` = $(@bind tol Slider(10.0 .^ (-8:-5), default = 1.0e-5, show_value = true))
"""

# ╔═╡ 0c78401a-a924-4991-a7ad-d2744dcbf59a
md"""
...
# ╔═╡ 9667da60-92be-4e44-ae14-9469b1747eba
space

# ╔═╡ 174168a8-34d0-4c78-b79e-4b98a8a17058
space

# ╔═╡ 78b9144c-281a-4db2-bf76-174e9b2a69d7
space

# ╔═╡ d6dd8bee-73b8-444d-8838-7dff17e9537b
space

# ╔═╡ 72d42b50-345f-45b7-a12f-f39ca45befa4
...
# ╔═╡ d71305f1-dfa4-4bc5-95eb-ff7a95285ceb
ode_arenstorf = let
...
# ╔═╡ fc212829-9070-495d-91ab-f609d1b41ffa
let
	sol = solve(ode_arenstorf, Tsit5(); 
				controller = IController(), abstol = tol, reltol = tol)
	plot(sol; idxs = (3, 4), xguide = L"x", yguide = L"y",
		 linewidth=2, labels="", title="Trajektorie eines Satelietens")
	scatter!([sol.u[1][3]], [sol.u[1][4]], label = L"x(0), y(0))")
end

# ╔═╡ 76d0a1f5-fc84-4ff6-a1aa-73352faf2ad8
let
	sol = solve(ode_arenstorf, Tsit5(); 
				controller = IController(), abstol = tol, reltol = tol)
	scatter(sol.t[begin:end-1], diff(sol.t);
			label = "", xguide = "Zeit \$ t \$", yguide = "Schrittweite \$ h \$",
			yscale = :log10)
end
...
let
	...
	scatter(last.(errors_rhs_fixed), first.(errors_rhs_fixed);
			xscale = :log10, yscale = :log10, label = "Uniforme Schrittweiten",
			xguide = "Anzahl RHS Auswertungen", yguide = "Fehler nach einer Periode")
	
	tol = 10.0 .^ range(-10, -2, length=20)
	errors_rhs_adaptive = compute_error_adaptive.(tol)
	scatter!(last.(errors_rhs_adaptive), first.(errors_rhs_adaptive);
			 label = "Adaptive Schrittweiten")
end

# ╔═╡ a69f9531-6ae1-4248-a8fd-a6f0994d7a4a
function analytic_sol(sol,e)
    q_0 = Float64[]
    q_1 = Float64[]
    errors = Float64[]
    i = 1
    for t in sol.t
        res = nlsolve(α -> (α[1] - e*sin(α[1]) - t), [0.0], xtol = 1e-9)
        α = res.zero[1]

        θ = 2.0 * atan(sqrt((1+e)/1-e) * tan(0.5*α))
        r = (1 - e^2) / (1 + e * cos(θ))
...
begin 		
	...
	e = 0.5
	initial_position = [1.0 - e, 0.]
	initial_velocity = [0., sqrt((1+e)/(1-e))]

	e2 = 0.
	initial_position2 = [1.0 - e2, 0.]
	initial_velocity2 = [0., sqrt((1+e2)/(1-e2))]
	...
	function plot_orbit(sol) 
	    q1 = getindex.(sol.u, 3)
	    q2 = getindex.(sol.u, 4)
	    plot(q1, q2, lab="Orbit", title="Kepler Problem Lösung", xlabel="", ylabel="",
			#xticks=:none, yticks=:none)
		)
	end
	
	function plot_first_integrals(sol, H, L,e)
	    p1 = getindex.(sol.u, 1)
	    p2 = getindex.(sol.u, 2)
	    q1 = getindex.(sol.u, 3)
	    q2 = getindex.(sol.u, 4)
		...
		#errors = analytic_sol(sol, e)
		
		#plot!([0.],[0.], lab="abs. Fehler auf der Lösung", color=:green)
		
		#plot!(twinx(), errors, lab=:none, color=:green, xticks=:none, ylabel="Fehler")
	end

	function plot_error(sol, e)
		errors = analytic_sol(sol, e)
		plot(errors, lab="abs. Fehler auf die Lösung", title="Fehlerplot",
			color=:orangered1)
	end
	
	analysis_plot(sol, H, L, e=e) = plot(plot_orbit(sol), plot_first_integrals(sol, H, L,e), size=(600,300))
	
	analysis_plot_error(sol, e) = plot(plot_orbit(sol), plot_error(sol,e), size=(600,300))
...
begin
	...
	sol_ROS3_err = solve(prob2, ROS3())
	sol_Tsit5_err = solve(prob2, Tsit5())

	tspan2_long = (0,300)
	prob2_long = DynamicalODEProblem(pdot, qdot, initial_velocity2, initial_position2, tspan2_long)
	sol_err_long = solve(prob2_long, Ruth3(), dt=1//10)
	print("")
end

# ╔═╡ ed83d9a6-80a2-479f-912b-d0ea6ac04032
analysis_plot_error(sol_Tsit5_err, e2)

# ╔═╡ 94ee4571-604e-4851-84da-ee32707aa153
analysis_plot_error(sol_ROS3_err, e2)

# ╔═╡ 2dc74cf3-86b1-4f4e-9edb-d3988b24a5f6
analysis_plot_error(sol_err, e2)

# ╔═╡ f320ce1d-c497-4085-8bdd-6f81956dc42f
analysis_plot_error(sol_err_long, e2)
...
manifest_format = "2.0"
project_hash = "fb89f400e92c1346b22af84911804dac231b5abd"
