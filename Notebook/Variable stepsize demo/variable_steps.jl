...
# ╔═╡ b0338770-0bc3-11ef-3c0a-45056b044cc1
using Random

# ╔═╡ 31215871-d5fb-44aa-8ee4-a8ba609589a5
using PlutoUI

# ╔═╡ 5f82e892-8dcc-4393-8d2f-351ac7edf3dc
using Plots

# ╔═╡ 197332bf-ffbc-41c0-a5db-23abd8e11f44
using LaTeXStrings

# ╔═╡ 9ecb7caf-b2ae-4c7b-92b4-7a762859779d
using OrdinaryDiffEq

# ╔═╡ b3cdce66-6f7a-4f70-b556-33276152de11
using LinearAlgebra

# ╔═╡ 4ef5653c-1591-4080-9229-f19fb775e179
using ForwardDiff

# ╔═╡ f0383526-4bab-4ece-8e86-1d039ae6353d
md"""
...
md"""
# Symplektische Verfahren mit variabler Schrittweite
"""
...
begin 		
	...
	# divide H by 50 in order to be able to set integers as tstops later 
	pdot(dp,p,q,params,t) = ForwardDiff.gradient!(dp, q->-H(q, p)/50, q)
	qdot(dq,p,q,params,t) = ForwardDiff.gradient!(dq, p-> H(q, p)/50, p)
	...
	analysis_plot(sol, H, L, e=e) = plot(plot_orbit(sol), plot_first_integrals(sol, H, L,e), size=(600,300), layout=(1,2))
...
md"""
### Betrachten wir wieder das Keplerproblem von am Anfang, nur diesmal mit variabler Zeitschritte. 
Da symplektische Verfahren in `OrdinaryDiffEq` gedacht sind, nur mit konstanter Schrittweite genutzt zu werden, müssen wir etwas improvisieren.
"""

# ╔═╡ f29b51f1-aa90-4bf9-a836-10657d3b0042
let
	# Erstelle die konstanten Zeitschritte
	constant_stops = Vector(0:8:400)

	# Generiere "noise" um die konstanste Schrittweite zu varieren
	n = length(constant_stops) 
	noise = rand(-8:1:8, n)
	noise[1] = 0 # stelle sicher, dass die Anfangs- und Endpunkte passen
	noise[end] = 0
	# addiere die "noise" auf die konstanste Schrittweite
	variable_stops = constant_stops .+ noise
	
	plot(constant_stops, lab = "Konstante Schrittweite")
	plot!(variable_stops, ylabel = "skalierte Zeit", xlabel = "# Schritte", 
		lab = "Variable Schrittweite", title = "variable Schrittweiten")
end
...
analysis_plot(sol_NSF, H, L)

# ╔═╡ e8a95069-c715-4a67-8cbd-5957f83e59ac
md"""
### Obwohl sicher die Performance des Verfahrens etwas verschlechtert hat, zeigt es wieterhin dasselbe strukturelle Verhalten.
"""

# ╔═╡ 0da7cede-928d-4811-87bd-414323dff082
analysis_plot(sol_NSV, H, L)

# ╔═╡ bc38f079-8841-440e-8182-d6e87bca89cc
md"""
### Das steht im Kontrast zu unserem Symplektischen Verfahren. Hier scheinen die symplektische Eigenschaften verloren gegangen zu sein. 
"""

# ╔═╡ a1551cd5-2be5-4a71-8a01-3d7c3dcffdda
analysis_plot(sol_SF, H, L)

# ╔═╡ a9ce9359-1a8c-401f-8a54-90226de89906
analysis_plot(sol_SV,  H, L)
...
manifest_format = "2.0"
project_hash = "bfe1d0e42f51edb9ff598f36a9f4711adece1eb3"
